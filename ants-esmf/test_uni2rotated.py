...
tgt_cube = iris.load('rotated.nc', iris.Constraint(cube_func = lambda c: c.var_name == 'cellData'))[0]
...
print res_cube
iris.save(res_cube, 'uni_rotated.nc')
...
for s, cube in [('src', src_cube), ('tgt', tgt_cube), ('res', res_cube)]:
	...
	try:
		...
		lat = extractor.get_latitude()
		lon = extractor.get_longitude()
