...
def createData(filename, prefix):
    ...
    coordsPoint = pointCube.coords()
    lats1d = coordsPoint[0].points
    lons1d = coordsPoint[1].points
    lats = numpy.outer(lats1d, numpy.ones(lons1d.shape, lons1d.dtype))
    lons = numpy.outer(numpy.ones(lats1d.shape, lats1d.dtype), lons1d)
    data = cellCube.data
